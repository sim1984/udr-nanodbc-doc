...
Замечание. На моём компьютере со свеже установленной Windows 10 эта библиотека не запустилась. Причина отсутствие библиотеки `ucrtbased.dll` входящей в состав Windows 10 SDK.
Обычно она устанавливается вместе с Visual Studio.
...
Функции семейства `bind_<type>...` связывают значение с параметром, если СУБД поддерживает пакетные операции cм. `execute` batch_operations, то количество записанных значений не ограничивается, в разумных пределах. Само связывание происходит уже при вызове `execute`.
...
Функция `clear_bindings()` сбрасывает пакет параметров для пакетных операторов.
...
- `from_` - кодировка из которой надо перекодировать строку;
- `to_` - кодировка в которую надо перекодировать строку;
- `convert_size` - желаемый размер выходной строки. Если после перекодировки строка будет меньшего размера, чем задано в параметре `convert_size`, то она будет увеличена до значения этого параметра. 
...
Функция `unbind()` отвязывает буфер от заданного столбца.
...
Функция `execute_stmt()` подготавливает SQL запрос.  Параметры:
- `stmt` - указатель на запрос;
- `query` - текст SQL запроса;
- `timeout` - тайм-аут SQL оператора.
