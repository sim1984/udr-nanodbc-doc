...
Замечание. На моём компьютере со свежеустановленной Windows 10 эта библиотека не запустилась. Причина отсуствие библиотеки `ucrtbased.dll` входящей в состав Windows 10 SDK.
...
Функция `describe_parameter()` позволяет задать описание типа, размера и масштаб параметра.
- `stmt` - указатель на подготовленный запрос;
...
Функция `describe_parameters()` производит автоматическое описание всех параметров запроса на сове типов, которая вернула СУБД.
